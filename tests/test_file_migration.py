...
def test_migrate_to_next_version__when_invoked_on_a_file_that_is_already_the_latest_beta_1_version__then_it_returns_the_input_file_path():
    with tempfile.TemporaryDirectory() as tmp_dir:
        expected_filename = os.path.join(tmp_dir, "myfile.h5")
        test_file = MantarrayH5FileCreator(
            expected_filename, file_format_version=CURRENT_BETA1_HDF5_FILE_FORMAT_VERSION
        )
        test_file.close()
...
def test_migrate_to_next_version__when_invoked_on_a_0_3_1_file__then_the_new_file_has_features_of_0_4_1__including_raw_data_old_metadata_and_new_metadata():
    with tempfile.TemporaryDirectory() as tmp_dir:
        new_file_path = migrate_to_next_version(PATH_TO_GENERIC_0_3_1_FILE, working_directory=tmp_dir)
        ...
        np.testing.assert_array_equal(wf.get_raw_reference_reading(), old_wf.get_raw_reference_reading())
...
def test_migrate_to_next_version__when_invoked_on_a_0_4_1_file__then_the_new_file_has_features_of_0_4_2__including_raw_data_old_metadata_and_new_metadata():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        new_file_path = migrate_to_next_version(PATH_TO_GENERIC_0_4_1_FILE, working_directory=tmp_dir)
        ...
        # new metadata
        assert wf.get_h5_attribute(str(ORIGINAL_FILE_VERSION_UUID)) == str(NOT_APPLICABLE_H5_METADATA)
...
def test_migrate_to_latest_version__when_invoked_on_a_file_one_step_below_the_latest_beta_1_version__then_the_file_path_and_directory_are_passed_to_the_migrate_to_next_version_function(
    mocker,
):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        latest_version_file = MantarrayH5FileCreator(
            path_to_latest, file_format_version=CURRENT_BETA1_HDF5_FILE_FORMAT_VERSION
        )
...
def test_migrate_to_latest_version__when_invoked_on_a_0_3_1_file__then_the_returned_file_path_is_for_the_latest_version(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        path_to_latest = migrate_to_latest_version(PATH_TO_GENERIC_0_3_1_FILE, working_directory=tmp_dir)
